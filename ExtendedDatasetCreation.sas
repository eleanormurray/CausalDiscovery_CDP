###################################################
#This code cleans the public CDP dataset
#for analyses using the expert DAG covariate set
#from Debertin et al, 2023.
#The data cleaning restricts to the placebo arm
#SAS Code written by EJ Murray
####################################################


libname cdp '<path for input>';
libname autoDAG '<path for output>';
...
proc contents data = autoDAG.expertDAG;
run;

PROC EXPORT DATA= autoDAG.expertDAG
            OUTFILE= "<path>\cdp_expertDAG.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
